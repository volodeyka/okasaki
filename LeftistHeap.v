...
if h is T n x tl tr then (LE x tl) && (LE x tr) && (HeapOrder tl) && (HeapOrder tr) else true.


Fixpoint rk (h : Heap) : nat := if h is T _ _ _ b then (rk b).+1 else O.
...
Fixpoint rank (H : Heap) : nat := if H is T r _ _ _ then r else O.
...
Lemma case_Leftist_Inv_l n x tl tr : Leftist_Inv (T n x tl tr) -> Leftist_Inv tl.
Proof. by move=> /andP[/andP[]].  Qed.

Lemma case_Leftist_Inv_r n x tl tr : Leftist_Inv (T n x tl tr) -> Leftist_Inv tr.
Proof. by move=> /andP[/andP[]].  Qed.
